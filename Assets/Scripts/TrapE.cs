...
public class TrapE : MonoBehaviour {
    ...
    void Start ()
    {
        DoExplodeDamageE doDamage = GetComponentInChildren<DoExplodeDamageE>();
	...
	void Update ()
    {
        if (state == STATE_SIGNAL)
        {
            ...
            if (remainSignalTime <= 0)
            {
                ...
                GetComponentInChildren<DoExplodeDamageE>().explode();
    ...
    void OnTriggerEnter(Collider other)
    {
        if (state == STATE_WAIT)
        {
            ...
            if (relationOk && other.GetComponent<CharacterStatsEnemy>())
            {
                state = STATE_SIGNAL;
